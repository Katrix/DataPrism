...
import dataprism.sql.{NullabilityTypeChoice, NullabilityTypeChoiceArr, NullabilityTypeChoiceNoArr, Nullable, SelectedType, SqlNull}
...
object SkunkTypes {
  ...
  def arrayOf[A](tpe: SelectedType[Codec, A]): TypeOfN[A, tpe.Dimension] =
    ...
    NullabilityTypeChoice.notNullByDefaultDimensional(
      ...
      _.opt.imap[Seq[A] | SqlNull](_.getOrElse(SqlNull))(v => Nullable.syntax(v).toOption),
      tpe
    )
